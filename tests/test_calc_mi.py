...
from ssm_utils import calc_metabolic_index
...
class TestCalcMi(unittest.TestCase):
    ...
    def test_chinook_salmon(self):
        ...
        result = calc_metabolic_index(pO2, w, temperature, self.betas, self.var_covar, method="routine", confidence_level=0.95)  # Calculate MI with uncertainty
        ...
        result = calc_metabolic_index(pO2, w, temperature, self.betas, self.var_covar, method="smr", confidence_level=0.95)  # SMR has lower metabolic demands
        ...
        result = calc_metabolic_index(pO2, w, temperature, self.betas, self.var_covar, method="routine", confidence_level=0.95)  # Higher temperature increases metabolic demand
        ...
        result = calc_metabolic_index(pO2, w, temperature, self.betas, self.var_covar, method="routine", confidence_level=0.95)  # Larger fish have higher mass-specific metabolic demands
        self.assertGreater(1, result['mi'])
    ...
    def test_arrays(self):
        ...
        results_vectored = calc_metabolic_index(pO2_array, w_array, temp_array, self.betas, self.var_covar, 'routine')
        # Repeat each as a scalar calculation
        for i,(po2,w,temp) in enumerate(zip(pO2_array,w_array,temp_array)):
            r = calc_metabolic_index(po2, w, temp, self.betas, self.var_covar, 'routine')
