#!/usr/bin/env python
"""
Initialize and Load NetCDF Configuration - Salmon 100m Habitat
Module 1 of 14 in the analysis pipeline
Modified: 2026-02-12 - Changed from sole to salmon (all water column, not bottom only)
"""
...
#
# **Current Configuration:** Salmon - 100m depth limit, ALL WATER COLUMN
# **Changes from Sole:** Uses all water column layers (removed bottom-only masking at line ~366)
#
# ============================================================================
...
import yaml  #for loading configuration file
...
species_depth_limit_m = 100  # Changed from 80 (crab) to 100 for sole - 2026-02-12
...
layer_filter_mode = 'all_layers'  # Changed from 'bottom_only' for salmon - uses whole water column (wc)
#layer_filter_mode = 'bottom_only' # bottom layer only (benthic)
...
taxa = "salmon"  # Changed from sole to salmon - 2026-02-12 
#taxa = "sculpin"  
#taxa = "sole"
...
def load_all_nc_datasets(NetCDF_input_dir, subdirectories_to_load):
    """
    Load all NetCDF datasets from specified subdirectories within a  given directory.

    Parameters:
    NetCDF_input_dir (str): Path to the directory containing subdirectories with NetCDF files.
    ...
    for subdirectory in subdirectories_to_load:
        subdirectory_path = os.path.join(NetCDF_input_dir, subdirectory)  # Get the full path of the subdirectory
