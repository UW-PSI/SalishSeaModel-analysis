...
def create_TS_inlet_netcdf(nc_file, shp_file, output_dir, variable='netPP', inlet_name='Bellingham Bay'):
    ...
    if (ds[variable].ndim==3):
        # median
        median = ds[variable][:,:,idx].quantile(0.5, dim=['node',"siglay"], skipna=True)
        median = median.rename(f"{variable}_median")
        # lower bound
        zero_quantile = ds[variable][:,:,idx].quantile(0, dim=['node',"siglay"], skipna=True)
        q_zero = zero_quantile.rename(f"{variable}_quantile_0")
        # tenth quantile
        tenth_quantile = ds[variable][:,:,idx].quantile(.1, dim=['node',"siglay"], skipna=True)
        q_tenth = tenth_quantile.rename(f"{variable}_quantile_10")
        # ninetieth quantile
        ninetieth_quantile = ds[variable][:,:,idx].quantile(.9, dim=['node',"siglay"], skipna=True)
        q_ninetieth = ninetieth_quantile.rename(f"{variable}_quantile_90")
        # upper bound
        one_quantile = ds[variable][:,:,idx].quantile(1, dim=['node',"siglay"], skipna=True)
