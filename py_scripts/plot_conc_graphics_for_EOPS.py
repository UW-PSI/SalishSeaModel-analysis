...
def plot_conc_graphics(shp, case, model_var, stat_type, loc, run_file, frame="FullDomain"):
    ...
    # Load reDO_threshsults from scenario for 3D (water column) case
    else:
        try: 
            with xarray.open_dataset(run_file) as ds:
                param_full=ds[[*ds][0]]
    ...
    output_directory = pathlib.Path('/mmfs1/gscratch/ssmc/USRS/PSI/Rachael/projects/KingCounty/graphics/EOPS') 
    ...
    for day in range(ndays):
        ...
        output_file = output_directory/f'{run_tag}_{model_var}_{stat_type}_conc_{loc}_{model_day+2}_nolegend'
        
        # I'm not seeing a more skillful way of doing this...so...old school it is for now!
        if (model_date == pandas.Timestamp(date(2014,5,21))) or \
           (model_date == pandas.Timestamp(date(2014,8,25))) or \
           (model_date == pandas.Timestamp(date(2014,10,8))) or \
           (model_date == pandas.Timestamp(date(2014,12,29))):
            ...
            # ~~~ The next two lines added to send data to EOPS ~~~
            gdf_sm = gdf[[model_var,'geometry', 'lat','lon']]
            gdf_sm.to_file(f'{output_file}.geojson', driver='GeoJSON') 
            # ~~~ END ~~~
