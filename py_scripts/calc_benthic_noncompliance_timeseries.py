# Created by Rachael D. Mueller at the Puget Sound Institute with funding provided by King County
import sys
import os
import xarray
import openpyxl
...
def calc_benthic_noncompliant_TS(shp, case, noncompliance, human_allowance, model_var, run_file):
    """
    HEADER TO BE ADDED
    ...
    # # Initialize dictionaries
    DO_diff_lt_0p2={} # Boolean where DO<threshold
    volume_lt_0p2={} # 3D matrix of [0,1] where cells [are not, are] noncompliant
    ...
    #add noncompliant volume across depth-levels and nodes
    ...
    PercentNonCompliant_df = pandas.DataFrame(percent_volume_lt_0p2_TS_byRegion[run_type])
    
    return PercentNonCompliant_df, output_directory
...
if __name__=='__main__':
    ...
    PercentNonCompliant_TS_df,output_directory = calc_noncompliant_TS(
