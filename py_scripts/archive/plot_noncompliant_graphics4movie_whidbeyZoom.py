...
def plot_noncompliant_movie(shp, case, noncompliant, run_file):
    ...
    DO_diff_lt_0p2_wc[run_type]=DO_diff_lt_0p2[run_type].max(
        axis=1, initial=0)
    #/mmfs1/gscratch/ssmc/USRS/PSI/Rachael/projects/KingCounty/graphics/whidbey/DOXG/3b/movies_whidbeyZoom/noncompliance
    processed_netcdf_dir = pathlib.Path(ssm['paths']['graphics'])/case
    output_directory = processed_netcdf_dir/'noncompliance'/noncompliant_txt/'movies_whidbeyZoom'/run_type
    ...
    if os.path.exists(output_directory)==False:
        ...
        if os.path.exists(processed_netcdf_dir)==False:
            ...
            os.makedirs(
                ...
                exist_ok=True)
            os.makedirs(
                processed_netcdf_dir/'noncompliance'/noncompliant_txt, 
                mode=0o777,
                exist_ok=True)
            os.makedirs(
                processed_netcdf_dir/'noncompliance'/noncompliant_txt/'movies_whidbeyZoom', 
                mode=0o777,
                exist_ok=True)
            os.makedirs(
                processed_netcdf_dir/'noncompliance'/noncompliant_txt/'movies_whidbeyZoom'/run_type, 
                mode=0o777,
                exist_ok=True)
        ...
        elif os.path.exists(processed_netcdf_dir/'noncompliance'/noncompliant_txt)==False:
            os.makedirs(
                processed_netcdf_dir/'noncompliance'/noncompliant_txt, 
                mode=0o777,
                exist_ok=True)
            ...
            os.makedirs(
                ...
                mode=0o777,
                exist_ok=True)   
        elif os.path.exists(processed_netcdf_dir/'noncompliance'/noncompliant_txt/'movies_whidbeyZoom')==False:
            os.makedirs(
                processed_netcdf_dir/'noncompliance'/noncompliant_txt/'movies_whidbeyZoom', 
                mode=0o777,
                exist_ok=True)
    ...
    for day in range(ndays):
        ...
        if run_tag=='baseline':
            axs.set_title(f'2014 Conditions (Whidbey Region) \n non-compliant nodes for {model_date.month_name()} {model_date.day:02d}, 2014')
        else:
            if run_tag=='baseline':
                axs.set_title(f"{ssm['run_information']['run_description_short']['whidbey']['wqm_baseline']} (Whidbey Region)\n non-compliant nodes for {model_date.month_name()} {model_date.day:02d}, 2014")
            elif run_tag=='reference':
                axs.set_title(f"{ssm['run_information']['run_description_short']['whidbey']['wqm_reference']} (Whidbey Region)\n non-compliant nodes for {model_date.month_name()} {model_date.day:02d}, 2014")
            else:
                axs.set_title(f"{ssm['run_information']['run_description_short']['whidbey'][run_tag]} (Whidbey Region)\n non-compliant nodes for {model_date.month_name()} {model_date.day:02d}, 2014")
        axs.set_xticklabels('')
        axs.set_yticklabels('')
        output_file = output_directory/f'{case}_{run_tag}_all_noncompliant_wc_{model_day}.png'
...
if __name__=='__main__':
    ...
    REQUIREMENTS:
    - Configuration yaml (e.g. https://github.com/UWModeling/SalishSeaModel-analysis/blob/main/etc/SSM_config_whidbey.yaml)
